...
if(isset($_POST['Submit']))
{
  $id = $_GET['id'];
	...
	$update = "UPDATE `finalproject`.`qna` SET `question` = '".$question."', `answer` = '".$answer."' WHERE `id` = '".$id."'";
	$result = mysqli_query($conn, $update);
	
	if (!$result)
		{
			echo"<script>alert('Update Failed');  location='qna_edit.php';</script>";
		}
		else 
		{
			echo"<script>alert('Updated Successfully');  location='qna_edit.php';</script>";
		}
}	
else{
  $id = $_GET['id'];
	$sql = "SELECT * FROM `finalproject`.`qna` WHERE `id` = '$id'";
              ...
              <form method="POST"  >   
                ...
                <?php if(isset($fmsg)){ ?><div class="alert alert-danger" role="alert"> <?php echo $fmsg; ?> </div><?php } ?>	
  	            ...
  	            <label for="question" ><b>QUESTION:</b></label> <br>
	    	        <textarea rows="4" cols="80" type="text"  name="question" ><?php echo $row['question'];?></textarea>
                <br><br>
   	            <label for="answer"><b>ANSWER:</b></label><br>
    		       <textarea rows="4" cols="80" type="text"   name="answer" ><?php echo $row['answer'];?></textarea>
 <br>
 <br>
