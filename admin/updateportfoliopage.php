...
if (isset($_POST['photoname']))
{
  ...
  //$new = '../assets/img/portfolio/'.$fn.'.jpg';

  $sql ="UPDATE `tblphoto` SET `photoid`= '".$pi."', `photoname`= '".$pn."', `photodes`='".$pd."' ,
  `categoryID`='".$ci."'
  WHERE (`photoid`='".$pi."')LIMIT 1";  // sql command
  ...
  //$row=mysqli_fetch_assoc($result); 
    ...
    goto2("admin.php#portfolio","The Photo is Failed to be Updated");
	}else {
    goto2("admin.php#portfolio","Photo details is successfully updated");
		//echo"<script> alert('The Photo is Successfully Updated');  </script>";
	}
}else{
  $pi=$_GET['photoid'];
  $sql = "SELECT * FROM tblphoto where photoid='".$pi."'";
  mysqli_select_db($conn,"finalproject"); ///select database as default
  $result=mysqli_query($conn,$sql);  // command allow sql cmd to be sent to mysql
...
<head>
  ...
  <title>Update Info | ZHAZHA</title>
  ...
  <main id="main" data-aos="fade-up">
    ...
    <section class="inner-page">
      <div class="container">
        ...
        <form action="updateportfoliopage.php?photoid=<?php echo $pi;?>" method="POST">
    ...
    <tr>
        <td><label for="Photo Name"> Photo Name:</label></td>
        <td><input type="text" id="photoname" name="photoname"  
        style="width:500px" value="<?php echo $row["photoname"];?>"><br></td>
    </tr>
    <tr>
    <td><label for="Photo Description"> Photo Description:</label></td>
    <!--<td><input type="text" id="photodes" name="photodes"  
    style="width:500px" value="<?php //echo $row["photodes"];?>"><br></td>-->
    <td><textarea id="photodes" name="photodes"  
     rows="4" cols="59"><?php echo $row["photodes"];?></textarea></td>  
    </tr>
    <tr>
    <td><label for="Category ID">Current Category ID:</label></td>
    <td><input type="text" disabled id="categoryID" name="categoryID"  
    style="width:200px" value="<?php echo $row["categoryID"];?>"> <br></td>
    </tr>
    <tr>
    <td><label for="Category ID"> Category Name:</label></td>
    <td> <select  id="categoryID" name="categoryID"  style="width:200px" value="<?php echo $row["categoryID"];?>">
        <option value="1">1. Family</option>
        <option value="2">2. Wedding</option>
        </select>
   
        <br><br>  </td></tr>
        </table>
        <br>
  <input type="submit"  class="button" style="width:100px" value="Update">
  <input type="reset" class="button1" style="width:100px" value="Clear">
  <div class="backmain">
  <a href="admin.php"><input type="button" class="button" style="width:100px " value="Back"></a>
  </div>
</form>
