...
<head>
  ...
  <title>Update FAQ Title | ZHAZHA</title>
  ...
  <main id="main" data-aos="fade-up">
    ...
    <section class="breadcrumbs">
      <div class="container">
        ...
        <div class="d-flex justify-content-between align-items-center">
          <h2>Update FAQ Title</h2>
          <ol>
            <li><a href="admin.php">Home</a></li>
            <li>Update FAQ Title</li>
          </ol>
        </div>
    ...
    <?php
        ...
        if(isset($_POST['ptag'])) {
            ...
            $sql ="UPDATE `FAQ_title` SET `label`='".$ptag."' , `title1`='".$ptitle1."' , `title2`='".$ptitle2."' , `description`='".$desc."' WHERE (`faqID`='".$tagid."')";  // sql command
            mysqli_select_db($conn,"finalproject"); ///select database as default
            $result=mysqli_query($conn,$sql);  // command allow sql cmd to be sent to mysql
            if (!$result){
                //echo $teamid.$teamlabel.$title1.$des;
                    echo"<script> alert('The Title is Failed to be Updated'); window.location='admin.php#faq' </script>";
                }else {
                //echo "result: ";
                goto2("admin.php#faq","Title details is successfully updated");
            ...
            $sql ="select * from `FAQ_title` WHERE `faqID`='".$tagid."'";
            mysqli_select_db($conn, "finalproject");//select database as default
    ...
    <section class="inner-page">
      <div class="container">
      <form action="updateFAQtitle.php?faqID=<?php echo $tagid; ?>" method="POST">
        <div class="form-group">
          <label for="FAQ Tag">FAQ Tag:</label>
          <input type="text" class="form-control" id="ptag" name="ptag" value="<?php echo $row['label']; ?>"><br><br>
        </div>
        <div class="form-group">
          <label for="FAQ Title 1">FAQ Title 1:</label>
          <input type="text" class="form-control" id="ptitle1" name="ptitle1" value="<?php echo $row['title1']; ?>"><br><br>
        </div>
        <div class="form-group">
          <label for="FAQ Title 2">FAQ Title 2:</label>
          <input type="text" class="form-control" id="ptitle2" name="ptitle2" value="<?php echo $row['title2']; ?>"><br><br>
        </div>
        <div class="form-group">
          <label for="Description">Description:</label>
          <textarea class="form-control" id="desc" name="desc"><?php echo $row['description']; ?></textarea><br><br>
        </div>
        ...
        <div class="backmain">
        <a href="admin.php#faq"><input type="button" class="button" style="width:100px " value="Back"></a>
        </div>
    </form>
      </div>
    </section>
